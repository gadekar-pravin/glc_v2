...
def test_direct_owner_bootstrap_method_is_not_exposed():
    store = PairingStore()
...
def test_owner_paired_classification():
    store = PairingStore()
    rec = seed_owner(store, "webui", "owner-1")
...
def test_owners_only_returns_owner_paired():
    store = PairingStore()
    seed_owner(store, "telegram", "owner-1")
...
def test_revoke_removes_pairing():
    store = PairingStore()
    seed_owner(store, "matrix", "owner-1")
